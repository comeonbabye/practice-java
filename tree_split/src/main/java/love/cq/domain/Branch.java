...
public class Branch implements WoodInterface {
	...
	public WoodInterface add(WoodInterface branch) {
		...
		if (bs >= 0) {
			...
			case -1:
				this.branch.setStatus(1); //如果是删除，把它的状态改成不是一个词，但是可以继续
				break;
			case 1:
				if (this.branch.getStatus() == 3) {    
					this.branch.setStatus(2);  //如果插入的不是结尾，但是之前是确定的3状态，然后改成状态2，是个词语，还可以继续，？？？？？？？
				}
				break;
			case 3:
				if (this.branch.getStatus() != 3) {
					this.branch.setStatus(2);   //如果之前不是状态3，但是插入的是状态3，则改成状态2，是个词语，还可以继续
				}
				this.branch.setParam(branch.getParams()); //插入状态3，需要添加词性等参数
			}
			return this.branch;
		} else {//插入词，需要拷贝前半段和后半段
